...
\begin{document}
	...
	3. \textbf{时间复杂度}合并时两部分结果个数不超过2k，每个元素遍历依次数组，即合并复杂度为$O(n)$，
	递归产生两个子问题，子问题规模为原来一半，可列出递归方程为$T(n)=2T(\frac{n}{2})+O(n)$，根据主定理可得$T(n)=O(n\log n)$。\\
	$
	\label{Majority算法}
	\begin{aligned}
	\hline
	&\textbf{算法4 }Majority\text{ 算法}\\
	\hline
	...
	其节点总数为$fulln(h)=\sum_{i=0}^{h}2^i=2^{h+1}-1$，其所有节点的高度和为$fullh(h)=\sum_{i=0}^{h}2^i(h-i)=2^{h+1}-h-2$。\\
	\hspace*{20pt}对于完美二叉树，最底层每减少两个节点(即一个父节点变为叶节点)，节点高度和减少1，而节点个数减少2，可知当最层仅剩一个节点时，节点个数和高度和的差值最小。\\
	\hspace*{20pt}对于完美二叉树，若最底层加一个节点，对于最左侧节点其所有高度均加1，若原高度为h，则总高度增加h+1。\\
	\hspace*{20pt}根据堆的性质，堆的左右子树至少有一个为完美二叉树，可以看作由完美二叉树，从右向左删除最底层叶结点构成。\\
	可知当最底层仅有一个节点时，节点个数和所有节点高度和的差值最小。\\
	\hspace*{20pt}设堆高度为h，则节点个数和高度和差值为$$node(h)-height(h)\geq1+fulln(h-1)-fullh(h-1)-h=1$$
